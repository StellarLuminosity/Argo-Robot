...
from gym_environments.environments import TradingEnv
...
eval_env.add_metric('Position Changes', lambda history : np.sum(np.diff(history['position']) != 0) )
eval_env.add_metric('Episode Lenght', lambda history : len(history['position']) )
